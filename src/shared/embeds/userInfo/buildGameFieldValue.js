 ...
 * @param {Array<Object|string>} entries - 유저가 등록한 닉네임 배열
 * @returns {string} - 형식화된 닉네임 라인들을 줄바꿈(\n)으로 연결한 문자열
 */
function buildGameFieldValue(game, entries) {
    return entries.map(entrie => formatNicknameLine(game, entrie)).join('\n');
}
