...
function createFields(nickNames, guildData, emojiMap) {
    ...
    if (guildData.steam && nickNames.steam.length > 0) {
        ...
        fields.push({ name: 'Steam', value: steamLinkIncluded ? `[스팀 친구 추가](${removeSpaces(steamNickName)})` : `<:wave_steam:${steamEmoji}> ${steamNickName}` });
    };
...
module.exports = async (newState) => {
    try {
        ...
        // 이모지 맵 로드
        emojiMap = emojiMap || loadEmojiMap(newState);  // 이걸 24시간 마다 맵을 초기화 시키면 될듯?

        const fields = createFields(nickNames, guildData, emojiMap);

        const embed = customEmbed(member, fields, nickNames.updatedAt);
