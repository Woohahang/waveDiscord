...
function createMenuOptions(userData) {
    ...
    games.forEach(game => {
        ...
        if (userGameData && userGameData.length > 0) {
            ...
            userGameData.forEach(entry => {

                if (game == 'steam')
                    gameNickname = entry.playerName;

                else if (game == 'leagueOfLegends')
                    gameNickname = entry.summonerName;

                else gameNickname = entry;

                options.push({
                    value: `${game}:${gameNickname}`, // 옵션의 값은 "게임명:닉네임" 형식입니다.
                    label: gameNickname,  // 옵션의 라벨은 닉네임입니다.
