...
class GetShowableGamesUseCase {
    ...
    async execute({ guildId }) {
        ...
        if (!guildEntry)
            throw new Error('[GetShowableGamesUseCase] 길드 정보를 찾을 수 없습니다.');

        const disabledGames = guildEntry.getDisabledGames();

        if (disabledGames.length <= 0)
            return Result.ok({ code: STATE_KEYS.MENU.NO_MENU_TO_SHOW });

        return Result.ok({ data: disabledGames });
    }
}

module.exports = GetShowableGamesUseCase;