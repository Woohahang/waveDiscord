...
function createFields(userDocument) {
    ...
    if (userDocument.steam) {
        ...
        fields.push({
            name: 'Steam',
            value: includesSteamCommunity ? `[스팀 친구 추가](${removeSpaces(userDocument.steam)})` : userDocument.steam,
            inline: false
        });
    }
    ...
    if (userDocument.kakao) {
        ...
        fields.push({
            name: '카카오 배틀 그라운드',
            value: `[${userDocument.kakao}](https://dak.gg/pubg/profile/kakao/${removeSpaces(userDocument.kakao)})`,
            inline: false
        });
    }
